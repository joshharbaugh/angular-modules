...
ngGridFilters.filter('ngColumns', function () {
    return function (input) {
        angular.forEach(input, function(col, i) {
            if (col.isHidden) {
                input.splice(i, 1);
            }
        });
...
***********************************************/
ng.defaultGridTemplate = function(){ return '<thead><tr><td colspan="99"><div ng-show="showColumnMenu" class="btn-group pull-left"><a data-target="#" data-toggle="dropdown" class="btn white-btn dropdown-toggle" role="button">Choose Columns <span class="caret"></span></a><ul class="dropdown-menu"><li ng-repeat="col in columns | ngColumns"><label><input type="checkbox" class="ngColListCheckbox" ng-model="col.visible"/>{{col.displayName}}</label></li></ul></div><div ng-show="showFilter" class="pull-right input-prepend"><span class="add-on"><i class="icon-search"></i></span><input placeholder="Search" type="search" ng-model="filterText"/></div></td></tr><tr ng-header-row></tr></thead><tbody><tr ng-style="rowStyle(row)" style="position:relative;" ng-repeat="(i, row) in renderedRows" ng-click="row.toggleSelected($event)" ng-class="{\'selected\': row.selected}" ng-row></tr></tbody><tfoot ng-show="enablePaging"><tr class="ngPagerContainer"><td colspan="99" style="padding:0;"><ul class="pager"><li ng-class="{\'disabled\': cantPageBackward()}" class="pull-left" style="margin-left:10px;"><a ng-click="pageBackward()">Previous</a></li><li>{{pagingOptions.currentPage}} / {{totalPages}}</li><li ng-class="{\'disabled\': cantPageForward()}" class="pull-right" style="margin-right:10px;"><a ng-click="pageForward()">Next</a></li></ul></td></tr></tfoot>';};

/***********************************************
* FILE: ..\src\templates\rowTemplate.html
***********************************************/
ng.defaultRowTemplate = function(){ return '<td ng-repeat="col in visibleColumns()" class="col{{$index}} {{col.cellClass}}" ng-cell></td>';};

/***********************************************
* FILE: ..\src\templates\aggregateTemplate.html
***********************************************/
ng.aggregateTemplate = function(){ return '<td ng-click="row.toggleExpand()" colspan="99" class="dark"><h4>{{row.label CUSTOM_FILTERS}}</h4></td>';};

/***********************************************
* FILE: ..\src\templates\headerRowTemplate.html
***********************************************/
ng.defaultHeaderRowTemplate = function(){ return '<th ng-repeat="col in visibleColumns()" ng-click="changeSorting(col)" ng-class="selectedCol(col)" ng-header-cell>{{col.displayName}}<th>';};
...
ng.Column = function (config) {
    ...
    self.cellTemplate = function() {
        return colDef.cellTemplate || '<p>{{row.getProperty(col.field) CUSTOM_FILTERS}}</p>'.replace(CUSTOM_FILTERS, self.cellFilter);
    };
...
ng.Grid = function ($scope, options, gridDim, sortService) {
    ...
    self.filteredData = [];
    self.hiddenColumns = self.config.hiddenColumns;
    ...
    self.buildColumns = function () {
        ...
        if (self.config.displayEditButton) {
            columnDefs.push({
                field: 'Edit',
                displayName: ' ',
                width: self.elementDims.rowSelectedCellW,
                ...
                resizable: false,
                headerCellTemplate: '',
                cellTemplate: '<a class="btn btn-mini white-btn" ng-href="#/users/edit/{{row.entity[\'id\']}}">Edit</a>'
            });
        }
        if (self.config.displayDeleteButton) {
            columnDefs.push({
                field: 'Delete',
                displayName: ' ',
                width: self.elementDims.rowSelectedCellW,
                ...
                resizable: false,
                headerCellTemplate: '',
                cellTemplate: '<button class="btn btn-mini" ng-click="remove(row)">Delete</button>'
            });
        }
    ...
    $scope.visibleColumns = function () {
        return $scope.columns.filter(function (col) {
            angular.forEach(self.hiddenColumns, function(column) {
                if (column == col.field) {
                    col.isHidden = true;
